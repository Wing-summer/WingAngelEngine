  ...
  for (auto& [_, following] : candidates.tokens) {
    following.erase(
        std::remove_if(
            following.begin(),
            following.end(),
            [&](size_t token) { return ignoredTokens.find(token) != ignoredTokens.end(); }
        ),
        following.end()
    );
  }
...
bool CodeCompletionCore::translateStackToRuleIndex(
  ...
  const auto size = ruleWithStartTokenList.size();
  ...
  if (translateRulesTopDown) {
    // Iterate backward using reverse iterators
    return std::any_of(indices.rbegin(), indices.rend(), [&](size_t index) {
...
bool CodeCompletionCore::translateToRuleIndex(
  ...
  if (preferredRules.find(rwst.ruleIndex) != preferredRules.end()) {
    ...
    if (addNew) {
      candidates.rules[rwst.ruleIndex] = CandidateRule{rwst.startTokenIndex, path};
...
bool CodeCompletionCore::collectFollowSets(  // NOLINT
  ...
  for (const antlr4::atn::ConstTransitionPtr& transitionPtr : state->transitions) {
    ...
    if (transition->getTransitionType() == antlr4::atn::TransitionType::RULE) {
      ...
      followSets.push_back(FollowSetWithPath{allUserTokens(), ruleStack, {}});
    } else {
      ...
      if (!label.isEmpty()) {
        ...
        followSets.push_back(FollowSetWithPath{label, ruleStack, getFollowingTokens(transition)});
      }
    }
  }
  stateStack.pop_back();
...
CodeCompletionCore::RuleEndStatus CodeCompletionCore::processRule(  // NOLINT
  ...
  callStack.push_back(RuleWithStartToken{startTokenIndex, startState->ruleIndex});

  if (tokenListIndex >= tokens.size() - 1) {  // At caret?
    if (preferredRules.find(startState->ruleIndex) != preferredRules.end()) {
      ...
      for (const FollowSetWithPath& set : followSets.sets) {
        ...
        for (const size_t rule : set.path) {
          fullPath.push_back(RuleWithStartToken{
              startTokenIndex,
              rule,
          });
        }
        ...
        if (!translateStackToRuleIndex(fullPath)) {
          for (const size_t symbol : set.intervals.toList()) {
            if (ignoredTokens.find(symbol) == ignoredTokens.end()) {
              if (debugOptions.showDebugOutput) {
                std::cout << "=====> collected:  " << vocabulary->getDisplayName(symbol) << "\n";
              }
              if (candidates.tokens.find(symbol) == candidates.tokens.end()) {
  ...
  statePipeline.push_back(PipelineEntry{startState, tokenListIndex});
      ...
      switch (transition->getTransitionType()) {
        case antlr4::atn::TransitionType::RULE: {
          ...
          for (const size_t position : endStatus) {
            statePipeline.push_back(PipelineEntry{
        ...
        case antlr4::atn::TransitionType::WILDCARD: {
          if (atCaret) {
            if (!translateStackToRuleIndex(callStack)) {
              for (auto token = antlr4::Token::MIN_USER_TOKEN_TYPE; token <= atn->maxTokenType;
                   ++token) {
                if (ignoredTokens.find(token) == ignoredTokens.end()) {
